...
namespace PlantAll
{
    public partial class ModEntry
    {
        ...
        {
            return item != null && item.Stack > 0 && (item.Category == -74 || item.Category == -19) && !(item as Object).isSapling() && !Object.isWildTreeSeed(item.ParentSheetIndex.ToString());
        }
                ...
                if (
                    (
                        item.canBePlacedHere(location, new Vector2(tiles[i].X, tiles[i].Y)) &&
                         SHelper.Reflection.GetMethod(location, "CanPlantSeedsHere").Invoke<bool>(item.GetItemTypeId(), tiles[i].X, tiles[i].Y, false, "oops")
                    )
